...
#say DSL::General::DataQueryWorkflows::Grammar.subparse('separate the column "Variable" into Variable and Set with separator pattern ""');
...
@testCommands = ("filter with Package is 'Statistics' and Title starts with 'air'");
@testCommands = ("
load the dataset iris;
take from 1 to 120 with step 3;
group by the column Species;
show counts");

@testCommands = ("
load dataset rx/ COUNT .* titanic <wb>/;
filter by sex is 'female' and survived is 'died';
group by class and survived;
count;
take value");

@testCommands = ('DSL ® Raku::Reshapers;
load dataset rx/ iris <wb> /;
summarize data');

@testCommands = ("use data frame dfItemMatrix;
summarize the data;
group by Item and TagType;
mutate `Weight*Weight` as Square;
summarize with Norm = `sqrt(sum(Square))`;
assign pipeline value to dfNorms");

@testCommands = ("use data frame dfItemMatrix;
inner join with dfProf over TagType, Tag;
group by Item;
summarize with Score = `sum(Weight)`;
skim data;
keep the columns Score, Item;
sort by Score desc;
take top 10;
assign pipeline value to dfRecs");
...
#@testCommands = (
#'group by RowsCount; show counts',
#'group by RowsCount; show counts',
#'group by ColumnCounts; show counts; reverse sort',
#);

#@testCommands = (
#"convert to long form using the id columns ID1, ID2 and the variable columns V1 and V2"
#);

my @targets = ('English', 'Bulgarian', 'Korean', 'Spanish');
