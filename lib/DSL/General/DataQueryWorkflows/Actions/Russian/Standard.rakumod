...
use DSL::General::DataQueryWorkflows::Actions::Russian::Predicate;
use DSL::Shared::Actions::English::PipelineCommand;

unit module DSL::General::DataQueryWorkflows::Actions::Russian::Standard;

class DSL::General::DataQueryWorkflows::Actions::Russian::Standard
		is DSL::Shared::Actions::English::PipelineCommand
        is DSL::General::DataQueryWorkflows::Actions::Russian::Predicate {
	...
	method replace-missing-command($/) { make 'заменить отсутствующие значения на ' ~ $<replace-missing-rhs>.made; }
    ...
    method take-pipeline-value($/) { make 'взять объект'; }
    method echo-pipeline-value($/) { make 'эхо текущего значения'; }

    method echo-command($/) { make 'распечатать сообщение: ' ~ $<echo-message-spec>.made; }
